...
class Komut():
    def __init__(self, gelen_ses):
        self.ses = gelen_ses.upper()
        self.ses_bloklari = self.ses.split()
        self.komutlar = ["KOMUT", "YEMEK SİPARİŞİ", "TARİH", "YOUTUBE", "SPOTIFY", "NETFLIX", "WIKIPEDI", "ALIŞVERİŞ" ]
    ...
    def komut_say():
        x = "Komutlarımı çalıştırmak için isimlerini söylemen yeterli. İşte komut listem: Komut Adı: Kapat. Komut İşlevi: Kendimi kapatırım. \nKomut Adı: Sipariş. Komut İşlevi: Yemeksepeti üzerinden yemek siparişi verebilirim. \nKomut Adı: Tarih. Komut İşlevi: İstediğin bir günün tarihini söyleyebilirim."
        x1 = "Bernard: Komutlarımı çalıştırmak için isimlerini söylemen yeterli. İşte komut listem: \nKomut Adı: Kapat - Komut İşlevi: Kendimi kapatırım. \nKomut Adı: Sipariş - Komut İşlevi: Yemeksepeti üzerinden yemek siparişi verebilirim. \nKomut Adı: Tarih - Komut İşlevi: İstediğin bir günün tarihini söyleyebilirim."
        print(x1)
        Komut.konus(x)
    ...
    """def komut_bul(self):
        for sozcuk in self.ses_bloklari:
            if sozcuk in self.komutlar:
                komut_calistir(sozcuk)

    def komut_calistir(komut):
        if komut == "KOMUT SAY" or "KOMUTLARINI SAY" or "KOMUTLARINI SÖYLE":
            self.komut_say()"""
